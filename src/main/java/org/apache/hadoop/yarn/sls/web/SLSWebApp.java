...

  {
    ...
    try {
      simulateInfoTemplate = FileUtils.readFileToString(new File(
              cl.getResource("html/simulate.info.html.template").getFile()));
      simulateTemplate = FileUtils.readFileToString(new File(
              cl.getResource("html/simulate.html.template").getFile()));
      trackTemplate = FileUtils.readFileToString(new File(
              cl.getResource("html/track.html.template").getFile()));
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
